                      ...
                      <div key={item.id} className="flex items-center justify-between border-b py-2 last:border-0">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {item.calorias ? `${Math.round(item.calorias).toLocaleString()} kcal por unidad base` : "Sin aporte calórico relevante"}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold">{formatearCantidad(item.cantidad, obtenerUnidadItem(item.nombre, item.tipo))}</p>
                          <p className="text-sm text-muted-foreground">${(item.cantidad * item.porPrecio).toLocaleString()}</p>
                          <p className="text-xs text-muted-foreground">{Math.round(item.cantidad * (item.calorias || 0)).toLocaleString()} kcal</p>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Contacto: {participante.contacto?.trim() || "No registrado"}
                            </p>
                            {participante.metodoContacto === "correo" && <Mail className="h-4 w-4 text-muted-foreground" />}
                            {participante.metodoContacto === "telefono" && <Phone className="h-4 w-4 text-muted-foreground" />}
                          </div>
                        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between border-b py-2">
                    ...
                    <span className="font-semibold">${evento.proteinas.reduce((sum, p) => sum + p.cantidad * p.porPrecio, 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between border-b py-2">
                    <span className="text-muted-foreground">Total bebestibles</span>
                    <span className="font-semibold">${evento.bebestibles.reduce((sum, b) => sum + b.cantidad * b.porPrecio, 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between border-b py-2">
                    <span className="text-muted-foreground">Total insumos</span>
                    <span className="font-semibold">${evento.insumos.reduce((sum, i) => sum + i.cantidad * i.porPrecio, 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between border-b py-2">
                    <span className="text-muted-foreground">Total ensaladas</span>
                    <span className="font-semibold">${evento.ensaladas.reduce((sum, e) => sum + e.cantidad * e.porPrecio, 0).toLocaleString()}</span>
                  </div>
                  <Separator />
                  ...
                  <div className="flex justify-between rounded-lg bg-muted p-3 py-2">
                    ...
                    <span className="font-bold text-primary">{Math.round(caloriasTotales).toLocaleString()} kcal</span>
                  </div>
                  <div className="flex justify-between rounded-lg bg-muted p-3 py-2">
                    <span className="font-medium">Calorías por persona</span>
                    <span className="font-bold text-primary">{Math.round(caloriasPorPersona).toLocaleString()} kcal</span>
                  </div>
                  <div className="flex justify-between rounded-lg bg-muted p-3 py-2">
                  ...
                  <div className="flex justify-between rounded-lg bg-muted p-3 py-2">
                    ...
                    <span className="font-bold text-primary">${Math.round(costoPromedio).toLocaleString()}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="space-y-6">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-3">
                      <Button variant={evento.estado === "planificado" ? "default" : "outline"} className="w-full justify-start" onClick={() => handleCambiarEstado("planificado")}>
                        {evento.estado === "planificado" ? <CheckCircle2 className="mr-2 h-4 w-4" /> : <XCircle className="mr-2 h-4 w-4" />}
                        Planificado
                      </Button>
                      <Button variant={evento.estado === "confirmado" ? "default" : "outline"} className="w-full justify-start" onClick={() => handleCambiarEstado("confirmado")}>
                        {evento.estado === "confirmado" ? <CheckCircle2 className="mr-2 h-4 w-4" /> : <XCircle className="mr-2 h-4 w-4" />}
                        Confirmado
                      </Button>
                      <Button variant={evento.estado === "finalizado" ? "default" : "outline"} className="w-full justify-start" onClick={() => handleCambiarEstado("finalizado")}>
                        {evento.estado === "finalizado" ? <CheckCircle2 className="mr-2 h-4 w-4" /> : <XCircle className="mr-2 h-4 w-4" />}
                        Finalizado
                      </Button>
                    </div>
                  </CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div className="rounded-lg bg-muted p-4">
                        ...
                        <p className="text-sm text-muted-foreground">Total: ${Math.round(cotizacionSeleccionada.total).toLocaleString()}</p>
                      </div>
                      <div className="space-y-2 text-sm">
                        {cotizacionSeleccionada.detalles.slice(0, 6).map((detalle) => (
                          <div key={`${cotizacionSeleccionada.supermercado}-${detalle.nombre}`} className="flex justify-between">
