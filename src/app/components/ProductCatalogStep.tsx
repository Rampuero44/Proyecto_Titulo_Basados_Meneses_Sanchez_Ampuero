...
import { esProductoAlcoholico } from "../utils/alcohol";
...
export function ProductCatalogStep({ seleccionados, onChange, sidebarExtra, ocultarAlcohol = false }: Props) {
  ...
  const filteredProducts = productos.filter((p) => {
    ...
    if (ocultarAlcohol && esProductoAlcoholico(p)) return false;
    ...
    if (selectedCategory === "proteina" && selectedGrupo !== "all") {
      const tipoProducto = ((p as any).tipo ?? "").toLowerCase();
    ...
    <div className="flex gap-6 items-start">
      <div className="flex-1 space-y-6 min-w-0">
        ...
        {selectedCategory === "bebestible" && (
          <div className="flex flex-wrap gap-2">
            {Object.keys(ocultarAlcohol ? GRUPOS_BEBESTIBLE_SIN_ALCOHOL : GRUPOS_BEBESTIBLE).map((grupo) => (
              <button
                key={grupo}
